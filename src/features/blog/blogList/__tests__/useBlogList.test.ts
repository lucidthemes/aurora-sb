...
describe('useBlogList hook', () => {
  ...
  test('fetches posts data', async () => {
    ...
    const { result } = renderHookWithQueryClient(() => useBlogList({}));

    await waitFor(() => {
      expect(result.current.blogListQuery.data?.posts).toEqual(mockPosts);
      expect(result.current.blogListQuery.data?.postsCount).toEqual(3);
    });
